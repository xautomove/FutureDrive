...
"${pythonPath}" -u "${tempScriptPath}"`

            const wrapperScriptPath = commandExecutor.createTempFile(wrapperScriptContent, '.sh');
            ...
            const child = commandExecutor.executeFlow('bash', [wrapperScriptPath], {
                nodeId: node.id,
                env: cleanEnv,
                onStdout: (text) => {
                    if(GLOBALS.isDebug){
                ...
                onError: (error) => {
                    ...
                    commandExecutor.deleteTempFile(wrapperScriptPath);
